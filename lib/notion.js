...
const withNotionReadFallback = async (fallbackValue, callback) => {
  try {
    return await callback();
  } catch (error) {
    console.warn(`Notion read fallback: ${error.message}`);
...
export const getPublishedRecipesByType = async (type) => {
  ...
  return withNotionReadFallback([], async () => {
...
export const getRecipeBySlugFromNotion = async (type, slug) => {
  ...
  return withNotionReadFallback(null, async () => {
    const data = await notionRequest(`/databases/${process.env.NOTION_DATABASE_ID}/query`, {
      method: 'POST',
      body: JSON.stringify({
        filter: {
...
export const getRecipeBySlugForAdminFromNotion = async (type, slug) => {
  if (!isNotionConfigured()) {
    return null;
  }
...
export const getRecipesForAdminFromNotion = async (type = '') => {
  if (!isNotionConfigured()) {
    return [];
  }
...
export const createRecipeInNotion = async (recipeInput) => {
  if (!isNotionConfigured()) {
  ...
  const payload = {
    ...
    properties: buildRecipeProperties(recipeInput),
  };
...
export const updateRecipeInNotion = async (pageId, recipeInput) => {
  if (!isNotionConfigured()) {
    throw new Error('Notion is not configured');
  }
